...
class ReportCoverage {
    ...
    Configuration readSettings() {
        try {
            ...
            Configuration cfg = allSettings.tool['ReportCoverage'] as Configuration
            this.script.echo("cfg.coverage_file = ${cfg.coverage_file}")
            this.script.echo("cfg.fail_under = ${cfg.fail_under}")
            this.script.echo("cfg.fail_color = ${cfg.fail_color}")
            this.script.echo("cfg.warn_under = ${cfg.warn_under}")
            this.script.echo("cfg.warn_color = ${cfg.warn_color}")
            this.script.echo("cfg.pass_color = ${cfg.pass_color}")
            return cfg
        } catch (e) {
    ...
    void reportCoveragePercent(Object badge, String metric = LINE_METRIC) {
        try {
            ...
            this.script.echo("Error message: ${e.message}")
            this.script.echo("Missing property name: ${e.property}")
            this.script.echo("Class with Error: ${e.type.name}")
